...
function getImgHtml(index) {
  return `url(./img/bike${index}.jpg)`;
}

function openOverlay(i) {
  let overlay = document.getElementById("overlay");
  overlay.innerHTML = templateHtml(i);
  getOverlayImg(i);
  currentImage = i;
...
function getOverlayImg(index) {
  document.getElementById(`render_overlay`).style.backgroundImage =
    getImgHtml(index);
}
...
function switchImg(a, event) {
  ...
  document.getElementById(`render_overlay`).style.backgroundImage = getCurrentImg(currentImage);
  document.getElementById(`overlay_title`).innerHTML = `${imgTitle[currentImage - 1]}`;
  event.stopPropagation();
}

function getCurrentImg(currentImage){
  return `url(./img/bike${currentImage}.jpg)`;
}
